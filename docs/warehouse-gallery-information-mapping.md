...
`/warehouse-gallery` の詳細パネルは、次の5タブを基本形にする。
...
| 使いこなしレビュー | 一言でいうと、強み、注意、細分化レビュー | 折りたたみ可 |
| 作例 | 写りを確認するためのリンク導線 | 折りたたみ可 |

旧4タブ案の `レビュー・作例` は廃止し、`使いこなしレビュー` と `作例` に分離する。

現行 `/warehouse-gallery` は `AI解析`、`価格・購入`、`レビュー・作例` の3タブをデモ実装している。将来本番統合するなら、`基本情報` はタブ化するか、詳細パネル上部の固定サマリーとして出す。

docs 上の正式タブ名は `AI解析 / 基本情報 / 価格・購入 / 使いこなしレビュー / 作例` とする。実際の UI ではモバイル幅を考慮し、`AI解析 / 基本 / 価格 / レビュー / 作例` のような短縮ラベルも許容する。

5タブ化によりモバイル幅では横幅が厳しくなる可能性がある。実装時は、横スクロール、短縮ラベル、省略表示、またはアイコン併用などで破綻を避ける。
...
- 初期表示は `一言でいうと`、`強み`、`注意` のサマリー型にする。
- 解像 / ボケ / 収差 / 周辺減光 / 逆光 / 比較 / サイズ重量 / 根拠などの細分化レビューは、構造化データ整備後に扱う。
- 現時点では `/warehouse` の `LensDeepReviewPanel` 内の固定サンプルが中心であり、本番構造化データ化は今後の課題である。
...
- 作例は使いこなしレビューの補足ではなく、レンズ選びの最終判断材料として独立タブにする。
- 当面は Photo Yodobashi、Google 作例検索、海外レビューなどの `review_links` を中心にした軽いリンク集にする。
- Flickr は不可として検討済みのため、当面の作例導線には含めない。
- サムネイル画像化は当面しない。
- 作例タブは、画像で写りを確認するための導線として扱う。
- 作例そのものの収集・保存・転載は行わず、外部リンクまたは検索導線に留める。
...
- 作例導線を Photo Yodobashi / Google 検索 / 海外レビューリンクに留めるか、専用 source list を作るか。
- 5タブ構成をモバイルで横スクロールにするか、省略ラベルと折り返しで対応するか。

## 11. 次に決めるべきこと

1. `/warehouse-gallery` 詳細パネルの第1段階を、デモデータのまま5タブ構成へ整えるか。
...
1. `/warehouse-gallery` の詳細パネル UI をデモデータのまま5タブ前提で磨く。
