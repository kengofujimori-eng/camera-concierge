...
現在の優先タスクは、`/scene-playbooks` 上部の chooser intro を実際に選択できる UI にし、撮影シーンから detail / card へ自然に進めること。

今回の方針:

- `docs/current-task.md` を今回の実装内容に更新する。
- chooser intro の4項目を button 化し、主要3シーンは detail を開けるようにする。
- 旅行・おでかけは card-only のまま、カード位置へ誘導する。
- 既存の inline detail UI、4枚カード一覧、`data-testid` を保ったまま、必要最小限の UI 変更に留める。
