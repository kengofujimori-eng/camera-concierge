...
現在の優先タスクは、Scene Guide detail を「読む」長文から、撮影条件を選びながら候補を整理する「判断フロー」へ寄せること。

今回の方針:

- `docs/current-task.md` を今回の実装内容に更新する。
- まず家族写真ガイドだけを、共通前提、撮影条件の分岐、候補、総評、注意点の順で読める判断フローにする。
- 家族写真で判断フローの見え方を検証し、発表会 / 運動会は既存 detail 表示を維持する。
