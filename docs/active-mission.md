...
現在の優先タスクは、Scene Guide を card-only 一覧から、各シーンの撮影判断を深く読める detail panel へ発展させるための設計を固めること。

今回の方針:

- `docs/scene-guide-detail-panel-plan.md` を作成する。
- public beta 前は、まず inline expand で小さく検証する方針を整理する。
- Scene Guide detail は「この撮影シーンでどう選ぶか」を扱い、Deep Review の「このレンズをどう使うか」と主語を混ぜない。
- 今回は docs only とし、UI、静的データ、warehouse / chat / API / Dify には接続変更を加えない。
