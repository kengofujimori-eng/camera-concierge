...
現在の優先タスクは、主要3シーンの interactive UI で、撮影条件によって必要な焦点距離が変わることをより直感的に見せること。

今回の方針:

- `docs/current-task.md` を焦点距離 visualization の refinement 内容へ更新する。
- 焦点距離レールに、各焦点距離が使いやすい条件の意味ラベルを加える。
- 主候補を理由つきの焦点距離カードとして強く表示し、次点 / 安全策は補助チップとして残す。
- 独立した理由カードを主候補表示へ統合し、注意は補足メモとして軽くする。
- 家族写真 / 発表会の相談 handoff を維持し、運動会にはまだ追加しない。
- 旅行の card-only 表示を維持する。
