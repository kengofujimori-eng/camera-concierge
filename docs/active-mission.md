...
- 現在は、候補同士の違いを将来の Deep Review 比較へつなぐ comparison hooks pilot を進めている。
...
現在の優先タスクは、主要4シーンの主候補 / 次点 / 安全策から、迷いやすい比較テーマを短く示す Deep Review comparison hooks pilot を成立させること。

今回の方針:

- Scene Guide result に、具体商品ではなく比較テーマを表示する。
- 主候補 / 次点 / 安全策から、重複しない比較テーマを最大2件導く。
- Deep Review 連携予定であることを明示し、実遷移はまだ行わない。
- 既存の焦点距離レール、候補役割、レンズ条件、相談 handoff は維持する。
