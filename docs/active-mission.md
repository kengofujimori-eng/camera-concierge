...
現在の優先タスクは、主要3シーンの interactive UI を、説明を読む画面から条件を触って候補の変化が分かる画面へ磨くこと。

今回の方針:

- `docs/current-task.md` を Scene Guide interactive UI の polish 内容へ更新する。
- 家族写真 / 発表会 / 運動会に焦点距離レールを加え、条件変更による候補の動きを視覚化する。
- 主候補 / 次点 / 安全策は文章カードではなく、役割と焦点距離を中心に軽く表示する。
