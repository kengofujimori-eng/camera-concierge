...
現在の優先タスクは、Scene Guide を単なるカード一覧から、自分に近い撮影シーンを選びやすい入口へ育てること。

今回の方針:

- `/scene-playbooks` のカード一覧前に「どのガイドを見るべき？」という chooser intro を追加する。
- 家族写真、発表会、運動会、旅行・おでかけの4件を、撮影条件から選べるようにする。
- 既存カード、静的データ、ナビ、warehouse / chat / API / Dify には接続変更を加えない。
- スコアやランキングではなく、まず撮影条件を選ぶ導線として見せる。
