...
現在の優先タスクは、Scene Guide の card-only データを少しずつ増やし、撮影シーンから判断できる入口を育てること。

今回の方針:

- 4件目として `旅行・おでかけガイド` を追加する。
- 旅行では最高画質より、持ち出せること、交換しなくて済むこと、夜も撮れることを優先する。
- stable ID 未導入のため `relatedLensIds` は空配列を維持する。
- `/scene-playbooks` の表示コンポーネントやナビは触らず、静的データ追加で4枚表示にする。
