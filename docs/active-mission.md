...
現在の優先タスクは、Scene Guide を card-only 一覧から一歩進め、まず `家族写真ガイド` だけ inline detail を開けるようにすること。

今回の方針:

- `docs/current-task.md` を今回の実装内容に更新する。
- `family-photography` だけ optional detail data を追加する。
- `/scene-playbooks` では1つだけ inline detail を開けるようにする。
