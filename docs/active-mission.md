...
- 現在は Scene Guide / Resolver / Comparison の到達点を統合レビューとして記録し、次の pilot を選ぶ段階にある。
...
現在の優先タスクは、主要4シーンの interactive / handoff / Resolver / comparison hooks を統合レビューし、stable ID pilot、visual polish、Deep Review comparison format のどこへ進むか判断できる状態にすること。

今回の方針:

- `scene-guide-resolver-comparison-review.md` に現在の実装範囲と公開β判断を記録する。
- 次候補を stable ID pilot / visual polish / Deep Review comparison format として整理する。
- comparison hooks は placeholder のまま維持し、Deep Review への実遷移はまだ行わない。
