...
現在の優先タスクは、Scene Guide detail を「読む」長文から、撮影条件を選ぶと候補が絞られる体験へ寄せること。

今回の方針:

- `docs/current-task.md` を今回の実装内容に更新する。
- まず家族写真ガイドだけを、室内 / 屋外の条件選択で焦点距離候補を絞れる interactive decision flow にする。
- 家族写真で「条件を選ぶ」体験を検証し、発表会 / 運動会は既存 detail 表示を維持する。
- 旅行・おでかけは card-only のまま扱う。
- 既存の chooser、1つだけ detail を開く挙動、カード表示、`data-testid` を保つ。
