...
- 家族写真 / 発表会は、選択した条件を相談画面へ引き継ぐ handoff 対応済み。
...
現在の優先タスクは、Scene Guide の公開β確認結果を docs に固定し、次の接続タスクを選べる状態にすること。

今回の方針:

- `docs/scene-guide-public-beta-review.md` に、主要3シーン interactive と相談 handoff の現在地を記録する。
- 公開βブロッカーはないと判断し、未実装項目は non-blocking issue として分離する。
- 次候補は sports-day consultation handoff、軽微な UI polish、Lens Condition Resolver pilot とする。
- 今回はコード、warehouse / API / Dify / storage 仕様 / `public/lens_data.json` を変更しない。
