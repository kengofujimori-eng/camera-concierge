...
現在の優先タスクは、`家族写真ガイド` に続いて `発表会ガイド` にも inline detail data を追加し、Scene Guide detail の2件目を小さく検証すること。

今回の方針:

- `docs/current-task.md` を今回の実装内容に更新する。
- `recital-stage` だけ optional detail data を追加する。
- 既存の inline detail UI をそのまま使い、UI component や page は変更しない。
