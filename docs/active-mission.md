...
- 現在は既存ロジックを維持したまま、「読む」より「見て触って分かる」Scene Guide へ進める visual polish を計画している。
...
現在の優先タスクは、Scene Guide visual polish を小さなフェーズへ分け、最初の実装を VisualNote / info chips に限定できる状態にすること。

今回の方針:

- `scene-guide-visual-polish-plan.md` に visual first / no new logic / mobile first の方針を記録する。
- visual notes、scene icons、distance visualization、focal rail、mobile readability を段階化する。
- 最初の実装候補は、amber-heavy な補足表示を置き換える `VisualNote` とする。
