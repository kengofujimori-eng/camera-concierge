      ...
      <div className="flex flex-wrap gap-2">
        {choices.map((c) => (
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className="flex items-center gap-2 rounded-xl border border-[#5fc3e4]/30 bg-white/70 px-3 py-2 text-xs font-medium text-[#3f9fbd] shadow-sm shadow-blue-500/10 backdrop-blur transition-colors hover:border-[#5fc3e4]/70 hover:bg-[#5fc3e4]/10 dark:border-[#5fc3e4]/30 dark:bg-[#5fc3e4]/10 dark:text-[#9bdff1] dark:hover:bg-[#5fc3e4]/15"
          >
            <span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full bg-[#5fc3e4]/15 text-[10px] font-bold text-[#3f9fbd] dark:text-[#9bdff1]">
              {c.number}
            </span>
            {c.label}
          </motion.button>
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          <div className="h-6 w-6 rounded-md bg-slate-100 flex items-center justify-center flex-shrink-0 ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-white/10">
            <CameraIcon className="h-3.5 w-3.5 text-[#3f9fbd] dark:text-[#9bdff1]" />
          </div>
          {selected ? (
            ...
            <div className="pb-1.5 max-h-60 overflow-y-auto">
              {MOUNTS.map((m) => (
                ...
                >
                  ...
                  {selected?.id === m.id && (
                    <Check className="h-3.5 w-3.5 flex-shrink-0 text-[#5fc3e4]" />
                  )}
                </button>
              ))}
            </div>
...
// ── 予算スライダー ────────────────────────────────────────
const BUDGET_DOT_COLORS = ['#5fc3e4', '#76bfd8', '#9db8c7', '#c58ba9', '#e55d87']
  ...
  return (
    <div className="px-1">
        ...
        {BUDGETS.map((b, i) => (
          <span
            ...
            className={`text-[9px] font-medium transition-colors ${
              i === idx ? 'text-[#3f9fbd] dark:text-[#9bdff1]' : 'text-slate-500 dark:text-slate-400'
            }`}
          >
            {b.label}
          </span>
      ...
      <div className="relative h-5 flex items-center px-1">
        ...
        {idx >= 0 && (
          <div
            className="absolute left-1 h-0.5 rounded-full bg-[linear-gradient(90deg,#5fc3e4_0%,#e55d87_100%)] transition-all duration-200"
        ...
        <div className="relative w-full flex justify-between">
          {BUDGETS.map((b, i) => (
            <button
              ...
              style={i <= idx ? { backgroundColor: BUDGET_DOT_COLORS[i] ?? '#5fc3e4' } : undefined}
              className={`h-3.5 w-3.5 rounded-full border-2 transition-all duration-150 hover:scale-125 focus:outline-none ${
                i <= idx
                  ? 'border-white shadow-sm shadow-blue-500/10'
                  : 'bg-white border-slate-300 hover:border-[#5fc3e4]/80 dark:bg-slate-800/80 dark:border-slate-600 dark:hover:border-[#5fc3e4]/70'
              }`}
            />
          ))}
        </div>
    ...
    <div className="px-1">
      ...
      <div ref={trackRef} className="relative h-8 flex items-center select-none">
        ...
        <div className="absolute h-2 rounded-full"
          style={{ left: `${minPct}%`, width: `${maxPct - minPct}%`, background: active ? 'linear-gradient(90deg, #5fc3e4 0%, #e55d87 100%)' : 'transparent' }}
        />
        {/* Min ハンドル */}
        <div
          className={`absolute w-5 h-5 rounded-full border-2 cursor-grab active:cursor-grabbing shadow-md z-10 transition-colors ${
            active ? 'bg-[#5fc3e4] border-white shadow-[0_0_0_4px_rgba(95,195,228,0.18)] dark:bg-[#5fc3e4] dark:border-white/90 dark:shadow-[0_0_0_4px_rgba(95,195,228,0.24)]' : 'bg-white border-slate-300 hover:border-slate-400 dark:bg-slate-700 dark:border-slate-500 dark:hover:border-slate-400'
          }`}
        ...
        <div
          className={`absolute w-5 h-5 rounded-full border-2 cursor-grab active:cursor-grabbing shadow-md z-10 transition-colors ${
            active ? 'bg-[#e55d87] border-white shadow-[0_0_0_4px_rgba(229,93,135,0.16)] dark:bg-[#e55d87] dark:border-white/90 dark:shadow-[0_0_0_4px_rgba(229,93,135,0.22)]' : 'bg-white border-slate-300 hover:border-slate-400 dark:bg-slate-700 dark:border-slate-500 dark:hover:border-slate-400'
          }`}
      ...
      <div className="relative h-4 mt-0.5">
        {maxPct - minPct > 14 ? (
          <>
            <span className={`absolute text-[10px] font-mono font-semibold -translate-x-1/2 ${active ? 'text-[#3f9fbd] dark:text-[#9bdff1]' : 'text-slate-500 dark:text-slate-400'}`}
              style={{ left: `${minPct}%` }}>{display.minMm}</span>
            <span className={`absolute text-[10px] font-mono font-semibold -translate-x-1/2 ${active ? 'text-[#c34f73] dark:text-[#f3a6bd]' : 'text-slate-500 dark:text-slate-400'}`}
            ...
            return (
              <button
                ...
                className={`group rounded-xl p-[1px] text-left transition-all duration-150 focus:outline-none ${
                  selected
                    ? 'bg-[linear-gradient(90deg,#5fc3e4_0%,#e55d87_100%)] shadow-sm shadow-blue-500/10'
                    : 'bg-slate-200/80 hover:bg-[#5fc3e4]/20 dark:bg-white/10 dark:hover:bg-[#5fc3e4]/20'
                }`}
              >
                <span
      ...
      {(active || lensType !== 'auto') && (
        <p className="text-[10px] text-[#3f9fbd] dark:text-[#9bdff1] mt-1.5 leading-snug">
...
<div class="my-3 rounded-xl border border-slate-200/90 bg-white/95 px-4 py-3 shadow-sm shadow-slate-200/60 backdrop-blur dark:border-white/15 dark:bg-slate-900/80 dark:shadow-black/20">
  <div class="flex flex-wrap items-center gap-2">
    <span class="rounded-full bg-[#5fc3e4]/10 px-2.5 py-1 text-xs font-bold text-[#3f9fbd] dark:bg-[#5fc3e4]/15 dark:text-[#9bdff1]">${optionLabel}</span>
      ...
      <aside className="hidden md:flex flex-col w-[280px] flex-shrink-0 overflow-visible border-r border-slate-200 bg-white shadow-xl shadow-slate-900/5 dark:border-white/10 dark:bg-slate-950 dark:text-slate-100 dark:shadow-black/30">
        <div className="p-5 border-b border-slate-200 dark:border-white/10">
          <div className="flex items-center gap-2.5 mb-0.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-white/10">
              <CameraIcon className="h-4 w-4 text-[#3f9fbd] dark:text-[#9bdff1]" />
            </div>
        ...
        <div className="px-3 py-3 border-b border-slate-200 relative z-10 space-y-3 dark:border-white/10">
          ...
          {(selectedMount || bodyInput || selectedBudget || selectedFocal || lensType !== 'auto') && (
            <p className="px-1 text-[10px] text-[#3f9fbd] dark:text-[#9bdff1]">
              ✓ 質問に自動付与されます
            </p>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="space-y-1">
            {QUICK_QUESTIONS.map((q, i) => (
              ...
              >
                <ChevronRight className="h-3.5 w-3.5 flex-shrink-0 text-slate-400 transition-colors group-hover:text-[#3f9fbd] dark:text-slate-400 dark:group-hover:text-[#9bdff1]" />
        ...
        <div className="p-4 border-t border-slate-200 flex flex-col gap-2 dark:border-white/10">
          ...
          >
            <div className="flex items-center gap-2">
              <Warehouse className="h-4 w-4 text-slate-500 transition-colors group-hover:text-[#3f9fbd] dark:group-hover:text-[#9bdff1]" />
              <span>デジタル倉庫</span>
            </div>
            {warehouseCount > 0 && (
              <span className="rounded-full bg-[#5fc3e4]/10 px-2 py-0.5 text-xs text-[#3f9fbd] ring-1 ring-[#5fc3e4]/25 font-semibold tabular-nums dark:bg-[#5fc3e4]/15 dark:text-[#9bdff1] dark:ring-[#5fc3e4]/30">
                {warehouseCount}
              </span>
            )}
          </Link>
              ...
              >
                ...
                {!setupDone && (
                  ...
                  >
                    <div className="flex items-center gap-2.5 mb-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 ring-1 ring-slate-200 flex-shrink-0 dark:bg-[#5fc3e4]/10 dark:ring-[#5fc3e4]/30">
                        <CameraIcon className="h-5 w-5 text-[#3f9fbd] dark:text-[#9bdff1]" />
                      </div>
                      <div>
                        ...
                        return (
                          <button
                            ...
                            className={`group rounded-xl p-[1.5px] text-left transition-all duration-150 focus:outline-none ${
                              isSelected
                                ? 'bg-[linear-gradient(135deg,#5fc3e4_0%,#e55d87_100%)] shadow-md shadow-blue-500/10'
                                : 'bg-slate-200/80 hover:bg-[linear-gradient(135deg,#5fc3e4_0%,#e55d87_100%)] hover:shadow-md hover:shadow-blue-500/10 dark:bg-white/10 dark:hover:bg-[linear-gradient(135deg,#5fc3e4_0%,#e55d87_100%)]'
                            }`}
                          >
                            <span
                    ...
                    {selectedMount && (
                      ...
                      >
                        <div className="mb-2 flex items-center justify-between gap-3">
                          ...
                          {bodyInput.trim() && (
                            <span className="rounded-full border border-[#5fc3e4]/25 bg-white px-2 py-0.5 text-[10px] font-medium text-[#3f9fbd] dark:border-[#5fc3e4]/30 dark:bg-slate-950 dark:text-[#9bdff1]">
                              設定済み
                            </span>
                          )}
                        </div>
                ...
                <div className="text-center">
                  <div className="inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-white ring-1 ring-slate-200 mb-3 shadow-lg shadow-slate-200/70 dark:bg-white/[0.08] dark:ring-white/15 dark:shadow-black/20">
                    <CameraIcon className="h-7 w-7 text-[#3f9fbd] dark:text-[#9bdff1]" />
                  </div>
                  {selectedMount && (
                    <div className="mx-auto mb-3 inline-flex items-center gap-2 rounded-full border border-[#5fc3e4]/25 bg-[#5fc3e4]/10 px-3 py-1 text-xs font-medium text-[#3f9fbd] shadow-sm shadow-blue-500/10 dark:border-[#5fc3e4]/30 dark:bg-[#5fc3e4]/15 dark:text-[#9bdff1]">
                    ...
                    {SCENES.map((s, i) => {
                      ...
                      return (
                        <motion.button
                          ...
                          whileHover={{ y: -2 }}
                          whileTap={{ scale: 0.98 }}
                          className="group rounded-2xl bg-slate-200/80 p-[1px] text-left shadow-sm shadow-slate-200/60 transition-all hover:bg-[linear-gradient(135deg,#5fc3e4_0%,#e55d87_100%)] hover:shadow-lg hover:shadow-blue-500/10 focus:outline-none dark:bg-white/10 dark:shadow-none dark:hover:bg-[linear-gradient(135deg,#5fc3e4_0%,#e55d87_100%)]"
                        >
              ...
              {messages.map((msg) => (
                ...
                >
                  <div className={`flex-shrink-0 h-8 w-8 rounded-full flex items-center justify-center ${
                    msg.role === 'user' ? 'bg-[linear-gradient(220.55deg,#e55d87_0%,#5fc3e4_100%)] shadow-sm shadow-blue-500/10' : 'bg-white/90 ring-1 ring-slate-200/90 backdrop-blur dark:bg-white/[0.08] dark:ring-white/15'
                  }`}>
                    {msg.role === 'user'
                  ...
                  <div className="flex-1">
                    {msg.role === 'user' ? (
                      <div className="flex justify-end">
                        <div className="max-w-[85%] rounded-2xl rounded-tr-none border border-[#5fc3e4]/20 bg-[linear-gradient(220.55deg,#e55d87_0%,#5fc3e4_100%)] px-4 py-3 text-sm leading-relaxed text-white shadow-lg shadow-blue-500/10 whitespace-pre-wrap">
                          {msg.content}
                        </div>
                      </div>
                    ) : (
            ...
            <>
              ...
              >
                ...
                <div className="px-4 py-4 space-y-4">
                  ...
                  {(selectedMount || bodyInput || selectedBudget || selectedFocal || lensType !== 'auto') && (
                    <p className="px-1 text-[11px] text-[#3f9fbd] dark:text-[#9bdff1]">✓ 質問に自動付与されます</p>
                  )}
                  <button
                    onClick={() => setShowMobileSettings(false)}
                    className="w-full rounded-xl bg-[linear-gradient(220.55deg,#e55d87_0%,#5fc3e4_100%)] py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/10"
                  >
                    設定を保存して閉じる
                  </button>
                </div>
          ...
          <div className="mx-auto max-w-3xl">
            ...
            <div className="md:hidden flex items-center justify-between mb-2">
              ...
              <div className="flex flex-wrap items-center gap-1.5 flex-1 min-w-0">
                {selectedMount && (
                  <span className="inline-flex items-center gap-1 rounded-full border border-[#5fc3e4]/25 bg-[#5fc3e4]/10 px-2 py-0.5 text-[11px] text-[#3f9fbd] dark:text-[#9bdff1]">
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                ...
                <button
                  ...
                  className={`relative flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${
                    !selectedMount
                      ? 'bg-[#5fc3e4]/15 text-[#3f9fbd] ring-1 ring-[#5fc3e4]/30 dark:text-[#9bdff1]'
                      : 'bg-white/70 dark:bg-white/[0.06] text-slate-500 hover:text-[#3f9fbd] dark:hover:text-[#9bdff1]'
                  }`}
                  title="設定"
                >
                  ...
                  {/* 未設定バッジ */}
                  {!selectedMount && (
                    <span className="absolute -top-1 -right-1 h-2.5 w-2.5 rounded-full bg-[#5fc3e4] ring-2 ring-white dark:ring-slate-900" />
                  )}
                </button>
              </div>
            </div>
            ...
            <div className="mb-2 hidden items-center justify-between md:flex">
              <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400">
                {selectedMount && (
                  <span className="inline-flex items-center gap-1 rounded-full border border-[#5fc3e4]/25 bg-[#5fc3e4]/10 px-2 py-0.5 text-[#3f9fbd] dark:border-[#5fc3e4]/30 dark:bg-[#5fc3e4]/15 dark:text-[#9bdff1]">
                    <CameraIcon className="h-2.5 w-2.5" />{selectedMount.label}
                  </span>
                )}
                {bodyInput.trim() && (
            ...
            <div className="flex gap-3 items-end">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                transition={{ duration: 0.15 }}
                className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl bg-[linear-gradient(220.55deg,#e55d87_0%,#5fc3e4_100%)] text-white shadow-lg shadow-blue-500/10 transition-all hover:brightness-105 disabled:opacity-30 disabled:cursor-not-allowed disabled:shadow-none"
              >
